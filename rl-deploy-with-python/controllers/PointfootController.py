...
class PointfootController:
    ...
    #     # ===== ROS DEPTH LOGGER (THREAD) =====
    #     self.depth_bridge = CvBridge()
    #     self.latest_depth = None
    #     self.depth_last_t = 0.0
    #     self.depth_interval = 2.0  # 2 giây
    #     #self.depth_dir = "/home/guest/depth_logs"
    #     # self.depth_dir = os.path.join(os.path.dirname(__file__), "depth_logs")

    #     # os.makedirs(self.depth_dir, exist_ok=True)

    #     BASE_DIR = os.path.dirname(os.path.realpath(__file__))
    #     self.depth_dir = os.path.join(BASE_DIR, "depth_logs2")
    #     #os.makedirs(self.depth_dir, exist_ok=True)


    #     if not rospy.core.is_initialized():
    #         rospy.init_node("limx_depth_logger", anonymous=True, disable_signals=True)
    ...
    #     self._ros_thread = threading.Thread(target=_ros_spin, daemon=True)
    #     self._ros_thread.start()

    #     print("[DepthLogger] thread started")


    # def _depth_cb(self, msg):
    #     self.latest_depth = msg

    # def _depth_step(self):
    #     if self.latest_depth is None:
    #         return
    #     now = time.time()
    #     if now - self.depth_last_t < self.depth_interval:
    #         return
    #     self.depth_last_t = now

    #     depth = self.depth_bridge.imgmsg_to_cv2(self.latest_depth, desired_encoding="passthrough")
    #     ts = time.strftime("%Y%m%d_%H%M%S")
    #     path = os.path.join(self.depth_dir, f"depth_{ts}.npy")
    #     np.save(path, depth)
    #     print("[DepthLogger] saved", path)
    ...
    def compute_observation(self):
        ...
        obs = obs[:num_prop]
        # depth = np.full((11, 11), 0.20, dtype=float)
        # depth = depth.reshape(-1)       
        # print("Debug",obs)
        # print("Debug2",self.scaled_commands)
    ...
    def sensor_joy_callback(self, sensor_joy: datatypes.SensorJoy):
        ...
        # linear_x  = 0.8 if linear_x > 0.8 else (-0.7 if linear_x < -0.7 else linear_x)
        # linear_y  = 0.8 if linear_y > 0.8 else (-0.7 if linear_y < -0.7 else linear_y)
        # angular_z = 0.8 if angular_z > 0.8 else (-0.7 if angular_z < -0.7 else angular_z)

        # linear_x  = 0.8 if linear_x > 0.8 else (-0.5 if linear_x < -0.5 else linear_x)
        # linear_y  = 0.8 if linear_y > 0.8 else (-0.5 if linear_y < -0.5 else linear_y)
        # angular_z = 0.8 if angular_z > 0.8 else (-0.5 if angular_z < -0.5 else angular_z)


        self.commands[0] = linear_x * 0.7
        self.commands[1] = linear_y * 0.7
        self.commands[2] = angular_z * 0.5
        # self.commands[0] = linear_x * 1
        # self.commands[1] = linear_y * 1
        # self.commands[2] = angular_z * 1
